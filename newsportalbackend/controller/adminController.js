...
const adminLogin = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 1, msg: "Admin Login successfull", existAdmin });
  } catch (error) {
    console.error(error);
    res.status(400).json({ msg: "Some error occurs while login admin" });
  }
};
...
const adminUpdate = async (req, res) => {
  try {
    ...
    if (!findAdmin) {
      res.status(400).json({ status: 0, msg: "Admin not exist" });
      return;
    }
    ...
    const updtedAdmin = await Admin.findByIdAndUpdate(
      { _id: id },
      { name, emailid, password, loginid }
    );

    if (!updtedAdmin) {
      res.status(400).json({ status: 0, msg: "Admin Update unsuccessfull" });
      return;
    }
    ...
    res
      .status(400)
      .json({ status: 0, msg: "Some error occure while Updating admin" });
  }
};
